...
but is not a raw capture. It and `frames.jsonl` moved here from `0.153.4/`
after the generated 0.156.1 schema was compared with the upstream
`rust-v0.153.4` bundle: every definition they model (initialize, thread/start,
turn/start, Turn, agent-message delta, token usage, turn/completed,
thread/started, turn/started, SandboxPolicy) is unchanged or only gained
optional fields (`disabledPluginIds`, `Thread.originator`).

`app-server-start-smoke.jsonl` is a sanitized non-credentialed Windows
observation from 2026-09-24 for Codex CLI 0.156.1, captured with the same
`initialize`, `initialized` and `thread/start` requests the adapter sends. As on
0.153.4, the server omitted the `jsonrpc` member from both responses and
notifications, and returned `sandbox.type=readOnly` after a `workspace-write`
request; that permission mismatch remains unsupported. Unlike the 0.153.4
record, the capture also contains the `thread/started` notification that
follows the `thread/start` response, and the thread carries the new
`originator` and response `disabledPluginIds` fields. The 0.153.4 raw capture
was not repeated with the same capture window, so the extra notification is not
evidence of a server ordering change. The adapter therefore keeps the native
...
`version.txt` is the observed `codex --version` output.
...
In the pinned `rust-v0.156.1` protocol, unchanged from `rust-v0.153.4`,
`writableRoots` lists additional roots; the verified `cwd` is implicitly
writable. An explicit empty array is therefore
...
[core SandboxPolicy](https://github.com/openai/codex/blob/rust-v0.156.1/codex-rs/protocol/src/protocol.rs),
[v2 permission projection](https://github.com/openai/codex/blob/rust-v0.156.1/codex-rs/app-server-protocol/src/protocol/v2/permissions.rs),
[thread request](https://github.com/openai/codex/blob/rust-v0.156.1/codex-rs/app-server-protocol/src/protocol/v2/thread.rs),
and [request config loading](https://github.com/openai/codex/blob/rust-v0.156.1/codex-rs/app-server/src/config_manager.rs).

## Opt-in native transport check

[Recorded native transport evidence](native-smoke.md) identifies the exact
source revision and Windows permission-grant failure; Linux was not re-run for
0.156.1.
...
grant fails; it is not converted to a skip or a support claim. On the Windows
0.156.1 run the cleanup after that failure observed `OutputTruncated=false`; see
[native-smoke.md](native-smoke.md) for why this is not a Codex version change.
