...
// Quicktype gives an external root schema used as an array item a synthetic
// `Element` type even when the same root is already emitted as a named DTO.
// Reuse the canonical Evidence DTO so the batch does not create a second
// generated representation of the unchanged evidence contract.
const preserveGoEvidenceBatchItem = (contents) => {
  if (!contents.includes("[]SymmetryEvidenceV1Element")) return contents;
  const element = /type SymmetryEvidenceV1Element struct \{[\s\S]*?\n\}\n\n/u;
  if (!element.test(contents)) {
    throw new Error("quicktype Go output did not contain the expected evidence batch item type");
  }
  return contents
    .replace(/\[\]SymmetryEvidenceV1Element/gu, "[]SymmetryEvidenceV1")
    .replace(element, "");
};

// Adding response receipts makes quicktype rename the existing evidence kind
// enum to `ReceiptKind`. Keep the established generated name so the single
// evidence DTO remains source-compatible while receipts reuse its enum.
const preserveGoEvidenceKindName = (contents) =>
  contents.replace(/\bReceiptKind\b/gu, "SymmetryEvidenceV1Kind");
...
const generate = async (check = false) => {
  ...
  if (schemas.length !== 14) {
    throw new Error(`expected 14 v1 envelope schemas, found ${schemas.length}`);
  }
  ...
  const normalizedGo = preserveGoEvidenceKindName(
    preserveGoEvidenceBatchItem(
      preserveGoNullableMicrousd(
